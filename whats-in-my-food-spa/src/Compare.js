...
class Compare extends Component {
    ...
    isProductPresentInDb(prod) {
        if (prod !== "" && this.state.products.length > 0) {
            let products = this.state.products.filter((product)=>product.food_name.trim().toUpperCase() === prod.trim().toUpperCase());
    ...
    handleSubmit(e){
           ...
           let flag2 = this.isProductPresentInDb(this.state.product2);
            if (flag1 && flag2) {
                ...
                ToastNotification.showErrorMessage("bottom-center", "Both Products are not present in Database, Please search for a different Products");
            } else if (!flag1){
                ToastNotification.showErrorMessage("bottom-center", `${this.state.product1} not present in Database, Please search for a different Product`);
            } else {
                ToastNotification.showErrorMessage("bottom-center", `${this.state.product2} not present in Database, Please search for a different Product`);
            }
            e.preventDefault();
    }
